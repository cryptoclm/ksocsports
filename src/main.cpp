...
#if defined(NDEBUG)
#error "KICKSOCCER cannot be compiled without assertions."
#endif
...
int64_t GetDevelopersPayment(int nHeight) {
   if (nHeight <  Params().GetConsensus().height_last_PoW) {
        return COIN * 0;
    //} else if (nHeight < Params().GetConsensus().nSubsidyHalvingInterval && nHeight >= Params().GetConsensus().height_last_PoW) {
    //    return COIN * 0.5;
    } else {
        return COIN * 61;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from KICKSOCCER
...
{
    util::ThreadRename("kicksoccer-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Track zKSOC money supply in the block index
    if (!UpdateZPIVSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zKSOC supply for block=%s height=%d", __func__,
...
std::string GetWarnings(std::string strFor)
{
    ...
    if (fLargeWorkForkFound) {
        ...
        strStatusBar = strRPC = _("Warning: Your blockchain doesn't match the required checkpoints! You have forked, please resync KICKSOCCER Core.");
    }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // KICKSOCCER: We use certain sporks during IBD, so check to see if they are
        ...
        if (pfrom->cleanSubVer == "/KickSoccer Core:1.0.0.2/" || pfrom->cleanSubVer == "/KickSoccer Core:1.0.0.1/" ||
            pfrom->cleanSubVer == "/KickSoccer Core:2.0.0/" || pfrom->cleanSubVer == "/KickSoccer Core:2.0.1.2/")
        {
            version_old = "< 2.1.0.3";
...
int ActiveProtocol()
{
    // SPORK_15 is used for 70811 (v2.0.1+)
