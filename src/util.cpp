...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "kicksoccer" is a composite category enabling all KICKSOCCER-related debug output
            if (ptrCategory->count(std::string("kicksoccer"))) {
...
#else
    const char* pszModule = "KICKSOCCER";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\KickSoccer2
// Windows >= Vista: C:\Users\Username\AppData\Roaming\KickSoccer2
// Mac: ~/Library/Application Support/KickSoccer2
// Unix: ~/.KickSoccer2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KickSoccer2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "kicksoccer.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty kicksoccer.conf if it does not exist
        ...
        if (configFile != NULL){
            ...
            fprintf(configFile, "addnode=85.139.212.204\n");
            fprintf(configFile, "addnode=188.37.145.198\n");
            fprintf(configFile, "addnode=161.97.101.101\n");
            fprintf(configFile, "addnode=161.97.126.179\n");
            fprintf(configFile, "addnode=173.249.1.71\n");
            fprintf(configFile, "addnode=75.119.149.78\n");
            fclose(configFile);
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override kicksoccer.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "kicksoccerd.pid"));
