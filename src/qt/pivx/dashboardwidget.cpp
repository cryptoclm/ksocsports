...
DashboardWidget::DashboardWidget(KICKSOCCERGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    ui->labelMessage->setText(tr("Amount of KSOC earned via Staking & Masternodes"));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareKSOC, "square-chart-chnd");
    setCssProperty(ui->labelSquareMNRewards, "square-chart-mnrewards");
    setCssProperty(ui->labelKSOC, "text-chart-chnd");
    ...
    ui->labelAmountMNRewards->setText("0 KSOC");
    ui->labelAmountKSOC->setText("0 KSOC");
    setCssProperty(ui->labelAmountKSOC, "text-stake-chnd-disable");
    ...
    ui->btnHowTo->setText(tr("How to get KSOC"));
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair KSOC and MN rewards
...
void DashboardWidget::onChartRefreshed() {
    ...
    set1 = new QBarSet("MN");
    set0->setColor(QColor(236,179,85));
    set1->setColor(QColor(234,146,2));
    ...
    if (chartData->totalStakes > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountKSOC, "text-stake-chnd");
        setCssProperty(ui->labelAmountMNRewards, "text-stake-mnrewards");
    } else {
        setCssProperty(ui->labelAmountKSOC, "text-stake-chnd-disable");
        setCssProperty(ui->labelAmountMNRewards, "text-stake-mnrewards-disable");
    }
    forceUpdateStyle({ui->labelAmountKSOC, ui->labelAmountMNRewards});
    ui->labelAmountKSOC->setText(GUIUtil::formatBalance(chartData->totalStakes, nDisplayUnit) + " (Stakes)");
