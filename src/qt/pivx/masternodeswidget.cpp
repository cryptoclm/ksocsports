...
MasterNodesWidget::MasterNodesWidget(KICKSOCCERGUI *parent) :
    PWidget(parent),
    ...
    if (dialog->exportMN) {
        if (ask(tr("Remote Masternode Data"),
                tr("You are just about to export the required data to run a Masternode\non a remote server to your clipboard.\n\n\n"
                   "You will only have to paste the data in the kicksoccer.conf file\nof your remote server and start it, "
...
void MasterNodesWidget::onDeleteMNClicked()
{
    ...
    if (strConfFile != boost::filesystem::basename(strConfFile) + boost::filesystem::extension(strConfFile)) {
        throw std::runtime_error(strprintf(_("masternode.conf %s resides outside data directory %s"), strConfFile, strDataDirKS2));
    }
    ...
    if (boost::filesystem::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:20555 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked(){
    ...
    if (walletModel->getBalance() <= (COIN * GetMstrNodCollateral(chainActive.Height()))) {
        inform(tr("Not enough balance to create a masternode, 10,000 KSOC required."));
        return;
    }
    showHideOp(true);
