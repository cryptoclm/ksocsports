...
SendWidget::SendWidget(KICKSOCCERGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("KICKSOCCER address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 KSOC");
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress chndAdd = CBitcoinAddress(address.toStdString());
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(chndAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
