...
TopBar::TopBar(KICKSOCCERGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopKSOC}, "amount-small-topbar");
    setCssProperty({ui->labelAmountKSOC}, "amount-topbar");
    setCssProperty({ui->labelPendingKSOC, ui->labelLockedKSOC}, "amount-small-topbar");
...
void TopBar::loadWalletModel() {
    ...
    // update the display unit, to not use the default ("KSOC")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& lockedBalance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    /* KSOC Total */
    // KICKSOCCER excludes "locked" KSOC from the Available balance to improve UX
    CAmount chndAvailableBalance = balance - lockedBalance;
    QString totalKSOC = GUIUtil::formatBalance(chndAvailableBalance, nDisplayUnit);

    /* KSOC Available Balance */
    // Top
    ui->labelAmountTopKSOC->setText(totalKSOC);
    // Expanded
    ui->labelAmountKSOC->setText(totalKSOC);

    /* KICKSOCCER merged "Pending" and "Immature" into a single GUI balance, to simplify the experience for the user */
    // Locked
    ui->labelLockedKSOC->setText(GUIUtil::formatBalance(lockedBalance, nDisplayUnit));
    // Pending + Immature
    ui->labelPendingKSOC->setText(GUIUtil::formatBalance((unconfirmedBalance + immatureBalance), nDisplayUnit));
}
