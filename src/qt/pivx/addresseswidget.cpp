...
AddressesWidget::AddressesWidget(KICKSOCCERGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter KICKSOCCER address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress chndAdd = CBitcoinAddress(address.toUtf8().constData());
        if (walletModel->isMine(chndAdd)) {
            setCssEditLine(ui->lineEditAddress, false, true);
        ...
        if (walletModel->updateAddressBookLabels(chndAdd.Get(), label.toUtf8().constData(),
                chndAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
