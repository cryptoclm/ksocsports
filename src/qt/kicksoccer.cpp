...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in kicksoccer.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in kicksoccer.qrc)
...
/** Class encapsulating KICKSOCCER Core startup and shutdown.
...
/** Main KICKSOCCER application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (KICKSOCCERGUI)
...
bool BitcoinApplication::createWindow(const NetworkStyle* networkStyle)
{
    ...
    window = new KICKSOCCERGUI(networkStyle, 0);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(KICKSOCCERGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(KICKSOCCERGUI::DEFAULT_WALLET);
        ...
        // KICKSOCCER: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", KICKSOCCERGUI::tr("A fatal error occurred. KICKSOCCER can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(kicksoccer_locale);
    Q_INIT_RESOURCE(kicksoccer);
    ...
    /// 6. Determine availability of data directory and parse kicksoccer.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("KICKSOCCER Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("KICKSOCCER Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // kicksoccer: links repeatedly have their payment requests routed to this process:
    ...
    if (strWalletFile != boost::filesystem::basename(strWalletFile) + boost::filesystem::extension(strWalletFile)){
        throw std::runtime_error(strprintf(_("Wallet %s resides outside data directory %s"), strWalletFile, strDataDirKS2));
    }
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("KICKSOCCER Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
