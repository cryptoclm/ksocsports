...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter KICKSOCCER address (e.g. %1)").arg("ZmkEBAFKzay6fMaYp3ZBJGSDXCNaa9DjwD"));
...
{
    // return if URI is not valid or is no KICKSOCCER: URI
...
{
    // Convert kicksoccer:// to kicksoccer:
    //
    //    Cannot handle this later, because kicksoccer:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    ...
    /* Open kicksoccer.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KICKSOCCER.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for KICKSOCCER.lnk
...
{
    return GetAutostartDir() / "kicksoccer.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a kicksoccer.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=KICKSOCCER\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the KICKSOCCER app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add KICKSOCCER app to startup item list
