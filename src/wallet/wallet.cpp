...
/**
 * Fees smaller than this (in uKSOC) are considered zero fee (for transaction creation)
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 KSOC collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 KSOC collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 15000 KSOC.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT15000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 15000 KSOC.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-KICKSOCCER-address
                    CScript scriptChange;
        ...
        )
{
    ...
    if (!StakeableCoins(&vCoins)) {
        if (fDebug) LogPrintf("%s: No coin available to stake.\n", __func__);
        return false;
    }
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        // Calculate reward
        CAmount nReward;
        //Modified to match current KickSoccer createcoinstake
