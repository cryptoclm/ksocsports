...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    //    pblock->nVersion = 4; // Post-Zerocoin activation
        ...
        pblock->nVersion = 4; // The reverse hardfork, complete zerocoin deactivation and more.
...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zKSOC spends can have (had) very large priority, use non-overflowing safe functions
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("KICKSOCCERMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("KICKSOCCERMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("KICKSOCCERMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("kicksoccer-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running KICKSOCCERMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("KICKSOCCERMiner exception");
    } catch (...) {
        LogPrintf("KICKSOCCERMiner exception");
    }

    LogPrintf("KICKSOCCERMiner exiting\n");
}
