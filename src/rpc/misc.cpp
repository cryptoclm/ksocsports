...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total KICKSOCCER balance of the wallet\n"
            ...
            /*"  \"zerocoinsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zKSOC denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zKSOC denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zKSOC denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zKSOC denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zKSOC denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zKSOC denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zKSOC denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zKSOC denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zKSOC denominations\n"
            "  }\n"*/
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in KSOC/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in KSOC/kb\n"
    ...
    /*UniValue zchndObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zchndObj.push_back(Pair(std::to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zchndObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zerocoinsupply", zchndObj));*/
...
UniValue mnsync(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1 || (strMode != "status" && strMode != "reset" && strMode != "next")) {
        throw std::runtime_error(
            "mnsync \"status|reset\"\n"
            "\nReturns the sync status or resets or next sync.\n"
	...
	return "success to sync next updated";
    }
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"kicksocceraddress\"\n"
            "\nReturn information about the given KICKSOCCER address.\n"

            "\nArguments:\n"
            "1. \"kicksocceraddress\"     (string, required) The KICKSOCCER address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"kicksocceraddress\",    (string) The KICKSOCCER address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for KICKSOCCER cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: KICKSOCCER address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are KICKSOCCER addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) KICKSOCCER address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"kicksocceraddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"kicksocceraddress\"  (string, required) The KICKSOCCER address to use for the signature.\n"
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in kicksoccer.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in kicksoccer.conf\n"
            "  \"haveconnections\": true|false,     (boolean) whether network connections are present\n"
            "  \"mnsync\": true|false,              (boolean) whether the required masternode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeablecoins\": n                (numeric) number of stakeable UTXOs\n"
            "  \"stakingbalance\": d                (numeric) KSOC value of the stakeable coins (minus reserve balance, if any)\n"
